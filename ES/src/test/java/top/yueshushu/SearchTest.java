...
public class SearchTest {
	...
	public void likeTest() {
		FuzzyQueryBuilder fuzzyQueryBuilder = QueryBuilders.fuzzyQuery("title", "入门");
	...
	public void aggreTest() {
		...
		fuzzyQueryBuilder.fuzziness(Fuzziness.ONE);
		NativeSearchQuery nativeSearchQuery = new NativeSearchQuery(fuzzyQueryBuilder);
		nativeSearchQuery.addAggregation(AggregationBuilders.max("maxPrice").field("price"));
		nativeSearchQuery.addAggregation(AggregationBuilders.min("minPrice").field("price"));
		nativeSearchQuery.addAggregation(AggregationBuilders.avg("avgPrice").field("price"));
		nativeSearchQuery.addAggregation(AggregationBuilders.sum("sumPrice").field("price"));
		
		SearchHits<Product> search = elasticsearchRestTemplate.search(nativeSearchQuery, Product.class);
		showInfo(search);
		AggregationsContainer<?> aggregations = search.getAggregations();
		assert aggregations != null;
		log.info("聚合：{}", aggregations);
	}
