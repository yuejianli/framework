...
import top.yueshushu.idem.utils.AssertUtil;
 ...
 */
@Slf4j
public final class IdemSpiLoader<S> {
    ...
    private static final ConcurrentHashMap<String, IdemSpiLoader> SPI_LOADER_MAP = new ConcurrentHashMap<>();
     ...
     */
    public static <T> IdemSpiLoader<T> of(Class<T> service) {
        ...
        IdemSpiLoader<T> spiLoader = SPI_LOADER_MAP.get(className);
        if (spiLoader == null) {
            synchronized (IdemSpiLoader.class) {
                spiLoader = SPI_LOADER_MAP.get(className);
                if (spiLoader == null) {
                    SPI_LOADER_MAP.putIfAbsent(className, new IdemSpiLoader<>(service));
                    spiLoader = SPI_LOADER_MAP.get(className);
                }
            }
        }
    ...
    synchronized static void resetAndClearAll() {
        Set<Map.Entry<String, IdemSpiLoader>> entries = SPI_LOADER_MAP.entrySet();
        for (Map.Entry<String, IdemSpiLoader> entry : entries) {
            IdemSpiLoader spiLoader = entry.getValue();
    ...
    // Private access
    private IdemSpiLoader(Class<S> service) {
    ...
    public void load() {
        ...
        while (urls.hasMoreElements()) {
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    classList.add(clazz);
                    IdemSpi spi = clazz.getAnnotation(IdemSpi.class);
        ...
        Collections.sort(sortedClassList, new Comparator<Class<? extends S>>() {
            ...
            public int compare(Class<? extends S> o1, Class<? extends S> o2) {
                IdemSpi spi1 = o1.getAnnotation(IdemSpi.class);
                int order1 = spi1 == null ? 0 : spi1.order();

                IdemSpi spi2 = o2.getAnnotation(IdemSpi.class);
    ...
    /**
     * Throw {@link IdemSpiLoaderException} with message
     *
     * @param msg error message
     */
    private void fail(String msg) {
        log.error(msg);
        throw new IdemSpiLoaderException("[" + service.getName() + "]" + msg);
    }

    /**
     * Throw {@link IdemSpiLoaderException} with message and Throwable
     *
     * @param msg error message
     */
    private void fail(String msg, Throwable e) {
        log.error(msg, e);
        throw new IdemSpiLoaderException("[" + service.getName() + "]" + msg, e);
    }
}
